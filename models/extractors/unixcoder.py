"""UniXcoder embedding-based feature extractor."""
...
# From: https://raw.githubusercontent.com/microsoft/CodeBERT/master/UniXcoder/unixcoder.py
...
class UniXcoderExtractor(HuggingFaceExtractor):
    """Extracts 768-dim embeddings from microsoft/unixcoder-base."""

    cache_name: str = "unixcoder"

    def __init__(
        self,
        device: torch.device,
        embeddings_path: str,
        bert_model: str = "microsoft/unixcoder-base",
        batch_size: int = 16,
    ):
        self.device = device
        ...
        torch.manual_seed(2)
        self.model = UniXcoder(bert_model)
        ...
        with torch.no_grad():
            for i in range(0, len(queries), batch_size):
                ...
                tokens_ids = self.model.tokenize(
                    batch, max_length=512, mode="<encoder-only>"
                )
                # Pad to same length within the batch
                max_len = max(len(t) for t in tokens_ids)
                tokens_ids = [
                    t + [self.model.config.pad_token_id] * (max_len - len(t))
                    for t in tokens_ids
                ]
                source_ids = torch.tensor(tokens_ids).to(self.device)
                _, batch_embeddings = self.model(source_ids)
