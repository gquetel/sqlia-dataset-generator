...
2. **Attack campaigns**: 6 campaigns per endpoint, each using a different sqlmap technique
3. **Two-phase attacks**:
...
Normal samples are generated by filling templates with values from dictionaries. The `attacks_ratio` configuration parameter determines:
- How many normal samples are included in the test set
- Training set size (approximately equal to the number of generated attacks)
...
Configuration options in `config.toml` include attack ratio, statement type distribution, MySQL connection parameters, and output paths.
